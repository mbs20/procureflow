...
export const AuditPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setError(null);
    fetch(
      `${import.meta.env.VITE_API_BASE_URL || ""}/api/v1/audit?page=${page}&page_size=25`,
      ...
      .then(setData)
      .catch((e) => {
        if (!controller.signal.aborted) setError(translateBackendError(e, t));
      })
      .finally(() => {
        ...
        <>
          {data.items.length === 0 ? (
            ...
            <ol className="space-y-3">
              {data.items.map((event) => (
                <li key={event.id} className="rounded border p-3 break-words space-y-1">
          ...
          <div className="flex gap-4">
            <button
              ...
              className="px-3 py-1.5 rounded border border-border text-xs font-semibold disabled:opacity-50"
            >
              {t("audit.previous")}
            </button>
            <span className="text-xs self-center font-mono">{page}</span>
            <button
