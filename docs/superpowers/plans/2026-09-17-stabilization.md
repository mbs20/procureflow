...
- [ ] Forms/review: reproduce native step mismatch and unsafe numeric handling; strict validation and regression tests. Diagnose PDF worker against production HTTP.
...
- [ ] Confirmed P1: source download, CSV export, effective lead time, global audit data, responsive navigation, worker health and runtime DNS.
- [x] Integration: synthetic API-to-award regression without bypassing extraction/scoring logic; bilingual coverage.
- [ ] Gates: full pytest, frontend unit/typecheck/build, Playwright/accessibility, migrations, Docker/live checks. Review changes, logical commits, push only when gates pass.
...
Local gates: 151 backend tests, 51 frontend tests, typecheck/build, 35 non-Docker Playwright tests plus 2 EN/FR audit scenarios pass. Runtime Docker and Alembic remain blocked by the unavailable Linux engine. Runtime configuration changes are tested statically, not yet confirmed over HTTP.
