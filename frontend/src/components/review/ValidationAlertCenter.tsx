  ...
  if (!hasCritical && !hasWarnings && discrepanciesCount === 0) {
    return (
      <div className="flex items-center gap-2 px-3.5 py-2.5 bg-emerald-950/30 border border-emerald-800/40 rounded-lg text-xs text-emerald-300">
        ...
        <span>{t("review.validationPassed", "Extraction passed all structural and arithmetic validation checks. Eligible for approval.")}</span>
      </div>
    );
  }
      ...
      {hasCritical && (
        ...
        >
          <div className="flex items-center gap-2 font-semibold text-rose-300">
            ...
            <span>{t("review.criticalIssuesTitle", "Critical Issues (Preventing Extraction Approval)")}</span>
          </div>
          ...
          <p className="text-[10px] text-rose-400/80 italic mt-0.5">
            {t("review.criticalIssuesDesc", "Resolve all negative/zero amounts or acknowledge mathematical discrepancies to enable approval.")}
          </p>
        </div>
      )}
        ...
        >
          <div className="flex items-center justify-between font-medium text-amber-300 text-[11px]">
            <div className="flex items-center gap-2">
              ...
              <span>{t("review.reviewAlertsTitle", { count: validationStatus.warnings.length, defaultValue: `Review Alerts & Advisory Warnings (${validationStatus.warnings.length})` })}</span>
            </div>
          </div>
            ...
            {validationStatus.warnings.map((warn, idx) => (
              ...
              >
                ...
                {onAcknowledgeWarning && (
                  ...
                  >
                    {t("review.acknowledgeBtn", "Acknowledge")}
                  </button>
                )}
              </div>
            ))}
